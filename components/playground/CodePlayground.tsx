...
const CodePlayground = () => {
  ...
  const reset = () => {
    resetRuntime();
    resetProject();
    toast.success(t("The current workspace was reset to its starter template."));
  };
  ...
  const fork = () => {
    ...
    if (!created) return;
    toast.success(t("Created {name}. The original stays unchanged.", { name: t(created.name) }));
  };
        ...
        <div className="relative flex flex-col justify-between gap-6 xl:flex-row xl:items-end">
          <div className="max-w-3xl">
            <div className="mb-3 flex flex-wrap items-center gap-2">
              <Badge className="bg-orange-700 text-white">{t("Code Lab")}</Badge>
              <Badge variant="outline" className="border-orange-300/70 text-orange-700 dark:text-orange-300">
                <ShieldCheckIcon /> {t("isolated runtime")}
              </Badge>
            </div>
            <h1 className="text-dark100_light900 text-3xl font-bold tracking-tight sm:text-4xl">
              {t("Run frontend code without leaving DevFlow")}
            </h1>
            <p className="text-dark400_light700 mt-3 max-w-2xl text-sm leading-6 sm:text-base">
              {t("JavaScript runs in a disposable Worker. HTML/CSS/JS renders in an opaque-origin iframe. Logs, failures, timeout, reset, and Fork are all part of one reproducible workspace.")}
            </p>
          </div>
      ...
      <section className="background-light900_dark200 light-border mb-4 rounded-2xl border p-3 shadow-sm">
        <div className="flex flex-col gap-3 2xl:flex-row 2xl:items-center 2xl:justify-between">
          <div className="flex min-w-0 flex-1 flex-col gap-3 md:flex-row md:items-center">
            ...
            >
              <SelectTrigger className="w-full md:w-56" aria-label={t("Select local workspace")}>
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                {projects.map((project) => (
                  <SelectItem key={project.id} value={project.id}>
                    {t(project.name || "Untitled workspace")}
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
            ...
            <div className="flex items-center gap-1 rounded-xl bg-muted p-1">
              <Button
                ...
                className={currentProject.mode === "javascript" ? "bg-orange-700 text-white hover:bg-orange-800" : ""}
              >
              ...
              <Button
                ...
                className={currentProject.mode === "web" ? "bg-orange-700 text-white hover:bg-orange-800" : ""}
              >
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            >
              <SelectTrigger size="sm" aria-label={t("Execution timeout")}>
                <Clock3Icon />
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                {timeoutOptions.map((value) => (
                  <SelectItem key={value} value={String(value)}>
                    {t("{seconds}s timeout", { seconds: value / 1_000 })}
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
            ...
            <AlertDialog>
              ...
              <AlertDialogContent>
                <AlertDialogHeader>
                  <AlertDialogTitle>{t("Reset this workspace?")}</AlertDialogTitle>
                  <AlertDialogDescription>
                    {t("Its files will be replaced by the current mode's starter template. Fork first if you want to keep this version.")}
                  </AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>
                  <AlertDialogCancel>{t("Cancel")}</AlertDialogCancel>
                  <AlertDialogAction onClick={reset}>{t("Reset files")}</AlertDialogAction>
                </AlertDialogFooter>
              </AlertDialogContent>
            </AlertDialog>
            ...
            <Button
              ...
              disabled={isRunning}
              onClick={execute}
              className="bg-orange-700 px-4 text-white hover:bg-orange-800"
            >
      ...
      {currentProject.mode === "javascript" ? (
            ...
            <div className="mb-3 flex items-center justify-between px-1">
              <div>
                ...
                <p className="mt-0.5 text-xs text-muted-foreground">
                  {t("Top-level await is supported; use sandboxResult(value) to display a result.")}
                </p>
              </div>
              <Badge variant="outline">{t("Worker thread")}</Badge>
            </div>
            <CodeEditor
              label="JavaScript"
              language="javascript"
        ...
        <>
            ...
            <section className="min-w-0 rounded-2xl border bg-card p-3 shadow-sm">
              ...
              >
                <div className="mb-3 flex flex-col justify-between gap-2 px-1 sm:flex-row sm:items-center">
                  ...
                  <Badge variant="outline">{t("3-file workspace")}</Badge>
                </div>
          ...
          <div className="mt-3 flex gap-2 rounded-xl border border-amber-200 bg-amber-50 p-3 text-xs leading-5 text-amber-900 dark:border-amber-900/60 dark:bg-amber-950/30 dark:text-amber-200">
            ...
            <p>
              {t("The iframe blocks same-origin storage, forms, popups, top-level navigation, and direct network APIs. Its timeout reliably handles asynchronous work, but a synchronous infinite loop may still block a browser renderer; use JavaScript mode when you need a hard CPU stop.")}
            </p>
          </div>
        </>
      )}
